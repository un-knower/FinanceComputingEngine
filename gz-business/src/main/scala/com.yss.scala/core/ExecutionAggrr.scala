...
import com.yss.scala.core.ExecutionAggrrContants._
...
/**
  ...
  *          目标数据库：SZSTOCK
  */
object ExecutionAggrr extends Serializable {
   ...
   // val exe = sc.textFile("hdfs://192.168.102.120/yss/guzhi/execution_aggr_tgwid_1_20180124.tsv")
    ...
    val result = exeDF.flatMap{
      case (key1,iterable) => {
        ...
        var execution=new  ListBuffer[ExecutionAggrrOriginalObj]()
        ...
        for (func <- iterable) {
          ...
          if (appId == "052") {
            ...
            val Exe = ExecutionAggrrOriginalObj(TransactTime, appId, ReportingPBUID, key, LastPx, LastQty, Side, AccountID, fileDate, sqbh, fzqbz("fzqbz"), fywbz("fywbz"), setCode("setCode"))
