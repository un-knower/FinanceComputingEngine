...
object ShenzhenStockExchangeTriPartyRepoETL {
  def main(args: Array[String]): Unit = {
    ...
    //原始数据
    val sjsjg: DataFrame = spark.sqlContext.dbfFile("F:/work/数据文件/sjsjg")
    val sjsmx: DataFrame = spark.sqlContext.dbfFile("F:/work/数据文件/sjsmx")

//    println(sjsjg.rdd.collect().toBuffer)

    //    sjsjg.printSchema()
    //    sjsmx.printSchema()

    val sjsmxFilterRDD = sjsmx.rdd.filter(row => {
      "20180709".equals(row.getAs[String]("MXCJRQ"))&&
    ...
    val sjsjgFilterRDD = sjsjg.rdd.filter(row =>{
      "20180709".equals(row.getAs[String]("JGCJRQ"))&&
...
//    println(sjsmxFilterRDD.collect().toBuffer)
    ...
    val MXvalueRDD = sjsmxFilterRDD.map(lines =>{
      ...
      val MXYHS: String = lines.getAs[String]("MXYHS")
      val MXJYJSF: String = lines.getAs[String]("MXJYJSF")
      ...
      val endDate = simpleDateFormat.parse(MXQTRQDate)
        ...
        val FCSGHQXTemp: BigDecimal = BigDecimal((endDate.getTime - startDate.getTime)./(24*60*60*1000)).setScale(2,RoundingMode.HALF_UP) //"SFCS"时的购回期限 = 购回日期 - 首期日期


        val FRZLV: BigDecimal = BigDecimal(MXCJJG)/100 // 回购利率
...
//    Util.outputMySql(MXvalueRDD.toDF(), "sjsmxETL-wmz")
//    Util.outputMySql(JGvalueRDD.toDF(), "sjsjgETL-wmz")

//    def NowDate(): String = {
//      val now: Date = new Date()
//      val dateFormat: FastDateFormat = new FastDateFormat("yyyy-MM-dd")
