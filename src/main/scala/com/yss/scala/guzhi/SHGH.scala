...
object SHGH {
  def main(args: Array[String]): Unit = {
    ...
    val dbfFileFrame: DataFrame = spark.sqlContext.dbfFile("C:\\Users\\YZM\\Desktop\\gh.dbf")
    ...
    val dsResult: Dataset[ShangHaiGuoHu] = sumDbfFileFrame.map(line => {
      ...
      //买金额
      var FBJE: BigDecimal = BigDecimal(0.0)
      ...
      if (BS == "B") FBYHS = (FBJE * rateYH).setScale(2, RoundingMode.HALF_UP)
      //卖印花税
      ...
      //买实付金额
      val FBSFJE: BigDecimal = (FBJE + FBJSF + FBZGF + FBGHF + FBYHS).setScale(2, RoundingMode.HALF_UP)
      //卖实收金额
