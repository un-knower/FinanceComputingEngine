...
package com.yss.source.spooldir;
...
public class ReliableSpoolingFileEventReader implements ReliableEventReader {
    ...
    private ReliableSpoolingFileEventReader(File spoolDirectory,
        ...
        Preconditions.checkNotNull(setXmlNode);
        Preconditions.checkNotNull(currentRecord);
    ...
    private void initialize() throws IOException {
        ...
        if (fileSuffixes.equalsIgnoreCase(".dbf")) {
            readDBF = new ReadDbf(new FileInputStream(file), currentRecord, csvSeparator, 10);
        } else if (fileSuffixes.equalsIgnoreCase(".xml")) {
            readXml = new ReadXml(xmlNode, new RandomAccessFile(file, "r"), 0, currentRecord, csvSeparator, 10);
        }
     ...
     * @author wangshuai
     * @date 2018/9/29 17:20
     * @description 调取相对应的文件方法转成csv
     */
    private List<Event> readDeserializerEvents(int numEvents) throws IOException {
        ...
        } else if (fileSuffixes.equalsIgnoreCase(".tsv")) {
            EventDeserializer des = currentFile.get().getDeserializer();
            events = des.readEvents(numEvents);
            for (Event event : events) {
                String body = new String(event.getBody(), Charset.forName("utf-8"));
                event.setBody(body.replaceAll("\t", csvSeparator).getBytes(Charset.forName("utf-8")));
            }
        } else {
            EventDeserializer des = currentFile.get().getDeserializer();
            events = des.readEvents(numEvents);
...
//            String filename = currentFile.get().getFile().getAbsolutePath();
            String filename = currentFile.get().getFile().getAbsolutePath().replace(spoolDirectory.getAbsolutePath(), "");
    ...
    public static class Builder {
        ...
        private String setXmlNode =
                SpoolDirectorySourceConfigurationConstants.DEFAULT_XML_NODE;
        private String setCurrentRecord =
                SpoolDirectorySourceConfigurationConstants.DEFAULT_CURRENT_RECORD;
        private String setCsvSeparator =
                SpoolDirectorySourceConfigurationConstants.DEFAULT_SEPARATOR;
        ...
        public ReliableSpoolingFileEventReader build() throws IOException {
            return new ReliableSpoolingFileEventReader(spoolDirectory, completedSuffix,
                    ...
                    consumeOrder, recursiveDirectorySearch, setXmlNode, setCurrentRecord, setCsvSeparator);
        }
    }
